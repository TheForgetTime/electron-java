...
import {browserViewMediateService} from './browserViewApi';
...
export = (socket: Socket) => {
    ...
    socket.on('webContents-insertCSS', async (id, isBrowserWindow, path) => {
        if (isBrowserWindow) {
            const browserWindow = getWindowById(id);
            if (browserWindow) {
                await browserWindow.webContents.insertCSS(fs.readFileSync(path, 'utf8'));
            }
        } else {
            ...
            if (view) {
                await view.webContents.insertCSS(fs.readFileSync(path, 'utf8'));
            }
        }
    });
