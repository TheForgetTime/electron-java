...
const browserViewApi = (socket: Socket) => {
    ...
    socket.on('createBrowserView', async (options) => {
        options = JSON.parse(options)
        if (!hasOwnChildren(options, 'webPreferences', 'nodeIntegration')) {
        ...
        if (options.proxy) {
            await browserView.webContents.session.setProxy({proxyRules: options.proxy});
        }
    ...
    function hasOwnChildren(obj, ...childNames) {
